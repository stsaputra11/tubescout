...
export default function Home() {
  ...
  return <main className="shell">
    ...
    <section className="results">
      <div className="resultsTop"><div><h2>Research Workspace <span>{videos.length}</span></h2><p>Metadata + TubeScout intelligence calculated from the selected videos.</p></div>{videos.length>0&&<div className="toolbar"><label className="searchBox"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search results"/></label><button onClick={exportXlsx}><FileSpreadsheet size={16}/> XLSX</button></div>}</div>
