...
export default function Home() {
  ...
  function exportCsv(){const csv=XLSX.utils.sheet_to_csv(XLSX.utils.json_to_sheet(exportRows()));const blob=new Blob([csv],{type:"text/csv;charset=utf-8"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=`tubescout-research-${new Date().toISOString().slice(0,10)}.csv`;a.click();URL.revokeObjectURL(url);}
  ...
  return <main className="shell">
    ...
    <section className="results">
      <div className="resultsTop"><div><h2>Research Workspace <span>{videos.length}</span></h2><p>Metadata + TubeScout intelligence calculated from the selected videos.</p></div>{videos.length>0&&<div className="toolbar"><label className="searchBox"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search results"/></label><button onClick={copyAll}><Copy size={16}/> Copy</button><button onClick={exportCsv}><Download size={16}/> CSV</button><button onClick={exportXlsx}><FileSpreadsheet size={16}/> XLSX</button></div>}</div>
    ...
    <footer><span>TubeScout · YouTube Competitor Research Tool</span><strong>Research patterns. Create original concepts.</strong></footer>
  </main>;
}
